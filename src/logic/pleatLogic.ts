...
import type { PricingData, TieredLookupRow } from "../data/PleatsData/pleatsDataTypes.js";
...
export const generatePleatPartNumber = (
  ...
  if (inputs.depth === 1 || inputs.depth === 2) {
    if (
      width <= standardThreshold.Width_Limit &&
      length <= standardThreshold.Length_Limit
    ) {
      secondaryCode = 1;
    } else if (
      width <= standardThreshold.Width_Limit &&
      length <= oversizeThreshold.Length_Limit
    ) {
      secondaryCode = 2;
    } else if (
      width <= oversizeThreshold.Width_Limit &&
      length <= standardThreshold.Length_Limit
    ) {
      secondaryCode = 2;
    } else if (
  ...
  const isExactAndWhole =
    inputs.isExact && inputs.widthFraction === 0 && inputs.lengthFraction === 0;
...
export const calculatePleatPrice = (
  ...
  const { partNumber, debugInfo: partNumberDebugInfo } =
    generatePleatPartNumber(inputs, pricingData);
  ...
  const isPartNumberValid =
    partNumber.startsWith("1") || partNumber.startsWith("2");
...
const calculateSecondaryCode = (depthForThresholds: 1 | 2 | 4): number => {
  ...
  const secondaryCodeFor1InchLogic =
    inputs.depth === 4
      ? calculateSecondaryCode(2)
      : secondaryCodeForActualDepth;

  debugInfo.priceCalculation.secondaryCodeForActualDepth =
    secondaryCodeForActualDepth;

  debugInfo.priceCalculation.secondaryCodeFor1InchLogic =
    secondaryCodeFor1InchLogic;

  if (secondaryCodeForActualDepth === -1 || secondaryCodeFor1InchLogic === -1) {
    return {
      partNumber: "Invalid depth threshold",
      price: 0,
      cartonQuantity: 0,
      cartonPrice: 0,
      isOversize: false,
  ...
  const widthString = fractionalWidthCode
    ? `${inputs.widthWhole}.${fractionalWidthCode}`
    : `${inputs.widthWhole}`;

  const lengthString = fractionalLengthCode
    ? `${inputs.lengthWhole}.${fractionalLengthCode}`
    : `${inputs.lengthWhole}`;
  ...
  if (specialPriceRow) {
    ...
    return {
      ...
      cartonPrice: 0,
      isOversize: false,
      notes: specialPriceRow.Override_Price,
      debugInfo,
    };
  }
  ...
  if (typeof listPrice !== "number") {
    debugInfo.priceCalculation.finalPrice =
      "Price not available for this configuration";
    ...
    return {
      ...
      cartonPrice: 0,
      isOversize: false,
      notes: "Price not available for this configuration",
      debugInfo,
    };
  }
